resource "aws_db_instance" "myapp_db" {
  ...
  vpc_security_group_ids = [aws_security_group.posweb_mydbsecurity_2024_sg.id]
  skip_final_snapshot  = true
}

resource "aws_security_group" "posweb_mydbsecurity_2024_sg" {
  name        = "posweb_mydb_2024"
  description = "Allow MYDB inbound traffic and all outbound traffic"
  vpc_id      = aws_default_vpc.default.id

  tags = {
    Name = "posweb_mydbsecurity_2024_sg"
  }
}

resource "aws_vpc_security_group_ingress_rule" "posweb_mydb_2024_allow_mysql" {
  security_group_id = aws_security_group.posweb_mydbsecurity_2024_sg.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 3306
  ip_protocol       = "tcp"
  to_port           = 3306
